...
function buildFinesFilterLabel() {
  ...
  if (filters.fines.state !== "ALL")
    labels.push(`State: ${filters.fines.state}`);
...
function dimAll(containerId) {
  document
    .querySelectorAll(`#${containerId} .bar-rect`)
    .forEach((b) => b.classList.add("dimmed"));
}
...
function updateFinesKPIs() {
  ...
  const speedFines = d3.sum(
    data.filter((d) => d.METRIC === "speed_fines"),
    (d) => d.FINES,
  );
